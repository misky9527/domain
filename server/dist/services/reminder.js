...
exports.manualExpiryCheck = manualExpiryCheck;
 ...
 */
function manualExpiryCheck(userId) {
    const db = (0, db_1.getDb)();
    const user = db.prepare("SELECT id, username, telegram_bot_token, telegram_chat_id FROM users WHERE id = ? AND telegram_bot_token != '' AND telegram_chat_id != ''").get(userId);
    if (!user) {
        return { count: 0, message: '未配置 Telegram，请先在设置页配置 Bot Token 和 Chat ID' };
    }
    const domains = db.prepare("SELECT id, name, expiration_date FROM domains WHERE user_id = ? AND expiration_date != ''").all(userId);
    if (domains.length === 0) {
            ...
            if (daysRemaining >= 0) {
                for (const threshold of THRESHOLDS) {
                    if (daysRemaining <= threshold) {
                        const alreadySent = db.prepare('SELECT id FROM reminder_log WHERE domain_id = ? AND threshold = ?').get(domain.id, threshold);
                        if (!alreadySent) {
                            db.prepare('INSERT INTO reminder_log (domain_id, user_id, threshold) VALUES (?, ?, ?)').run(domain.id, userId, threshold);
                        }
                        break;
                    }
                }
...
function checkExpirationReminders() {
    ...
    for (const user of users) {
        const domains = db.prepare("SELECT id, name, expiration_date FROM domains WHERE user_id = ? AND expiration_date != ''").all(user.id);
        if (domains.length === 0)
            continue;
        const today = new Date();
        today.setHours(0, 0, 0, 0);
        ...
        for (const domain of domains) {
            ...
            for (const threshold of THRESHOLDS) {
                ...
                // 记录已发送
                db.prepare('INSERT INTO reminder_log (domain_id, user_id, threshold) VALUES (?, ?, ?)').run(domain.id, user.id, threshold);
                break; // 一个域名一次只发一条
            }
        }
