...
function getDnsProviderFromMap(nsHost) {
...
let _getDb = null;
function setDbGetter(getter) {
    _getDb = getter;
}
function getDnsProvider(nsHost) {
    ...
    if (_getDb) {
        try {
            ...
            if (db) {
                ...
                const rows = db.prepare('SELECT domain, name FROM dns_providers').all();
 ...
 */
function extractNsInfo(records) {
