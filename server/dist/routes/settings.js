...
router.get('/telegram', (req, res) => {
    ...
    res.json({
        telegram_bot_token: user.telegram_bot_token ? (0, crypto_1.decrypt)(user.telegram_bot_token) : '',
...
router.put('/telegram', (0, permission_1.requirePermission)('setting:telegram'), (req, res) => {
    ...
    db.prepare('UPDATE users SET telegram_bot_token = ?, telegram_chat_id = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?')
        .run(telegram_bot_token ? (0, crypto_1.encrypt)(telegram_bot_token) : '', telegram_chat_id || '', req.user.id);
...
router.post('/telegram/test', (0, permission_1.requirePermission)('setting:telegram'), async (req, res) => {
    ...
    const ok = await (0, telegram_1.sendTelegramMessage)(decryptedToken, user.telegram_chat_id, '<b>✅ DomainKeeper 通知测试</b>\n\n如果收到这条消息，说明配置正确！');
    if (ok) {
