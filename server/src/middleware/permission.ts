...
export function requirePermission(perm: string) {
  return (req: AuthRequest, res: Response, next: NextFunction): void => {
    ...
    try {
      const perms: string[] = req.user.permissions ? JSON.parse(req.user.permissions) : [];
 ...
 */
export function getCompanyFilter(req: AuthRequest): { companyId: number | null; needsJoin: boolean } {
  if (!req.user) return { companyId: null, needsJoin: false };
