...
router.put('/:id', (req: AuthRequest, res: Response) => {
  const { name } = req.body;
  const db = getDb();
  const group = db.prepare('SELECT id, is_default FROM domain_groups WHERE id = ? AND user_id = ?').get(req.params.id, req.user!.id) as any;
  if (!group) {
...
router.delete('/:id', (req: AuthRequest, res: Response) => {
  ...
  const group = db.prepare('SELECT is_default FROM domain_groups WHERE id = ? AND user_id = ?').get(req.params.id, req.user!.id) as any;
  if (!group) {
    res.status(404).json({ error: '分组不存在' });
    return;
  }
